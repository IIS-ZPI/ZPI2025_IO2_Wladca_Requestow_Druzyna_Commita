...
def _extract_mid_floats(rates: list) -> list[CurrencyEntry]:
    values = []
    for index, item in enumerate(rates):
        mid = item["mid"]
        ...
        values.append(CurrencyEntry(mid=float(mid)))
    return values


def fetch_currency_data(currency: str, sessions: int) -> list[CurrencyEntry]:
